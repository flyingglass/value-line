...
### 🔴 raw 入库结构模板（不可违背）
**raw 文件按"头部元数据 → 摘要 → 正文"三段式入库**，统一模板见 `research-ingest` skill（`.codebuddy/skills/research-ingest/`）：
...
> 原文链接：<url>
> 作者：<作者/整理者>
> 发布时间：YYYY-MM-DD HH:MM（修改于…）
> 会议信息：<公司> <类型>，YYYY-MM-DD，约N分钟
> 参会管理层：<姓名（职务）>
> 原始来源：<材料来源>
...
- 头部元数据：来源 / 链接 / 时间 / 人物齐全，可追溯（链接强制保留，见上）
- 摘要：结构化分点 + **关键数据与核心结论加粗**，3 分钟读完核心（一句话核心、关键数据、核心矛盾、跟踪要点）
- 正文：**发言人加粗**；问答环节按 `Q1 主题（机构）` 编号小标题，便于定位；原文完整保留不丢失

> 2026-08-22：《泡泡玛特/2026-08-20-中期业绩发布会完整逐字稿.md》确立三段式模板 + 突出重点格式。不可再犯（只有标题无元数据 / 无摘要 / 丢原文 / 大段平铺无加粗无分点）。
