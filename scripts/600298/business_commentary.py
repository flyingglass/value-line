# -*- coding: utf-8 -*-
"""安琪酵母 600298 — VL Business + AI Commentary（数据驱动）"""
def build(stock, metrics, revenue_structure, years, cagr, spot):
    ...
    lt_debt = _num(ly.get("LT_DEBT") or 0)
    price = _num(spot.get("price", 0))
    # A股 spot.pe/pb/div_yield 常为 0，自行计算
    ...
    # Business 描述
    biz = (
        f"安琪酵母是亚洲第一、全球第二的酵母公司，主营酵母及深加工产品（面包酵母、酵母抽提物YE、酿酒酵母等），"
        f"产品出口170+个国家和地区。2025年营收{_fmt(rev, 0)}亿（同比{_dir(rev, py.get('OPERATE_INCOME'))}{r_abs:.1f}%），"
        f"归母净利润{_fmt(np_val, 0)}亿（{_dir(np_val, py.get('HOLDER_PROFIT'))}{n_abs:.1f}%）。"
        f"业务结构：{prod_str}。"
        f"国内市占率55%遥遥领先（乐斯福14%、英联马利11%），"
        f"发酵总产能49万吨，酵母产量45.6万吨。"
    )

    # P1: 业绩快照
    p1 = (
        f"2026年6月 — 安琪酵母营收{_fmt(rev, 0)}亿（{_dir(rev, py.get('OPERATE_INCOME'))}{r_abs:.1f}%），"
        f"归母净利润{_fmt(np_val, 0)}亿（{_dir(np_val, py.get('HOLDER_PROFIT'))}{n_abs:.1f}%）。"
        f"核心酵母及深加工收入119.5亿（+10.1%），销量46.8万吨（+15.8%），以量补价态势延续。"
        f"食品原料22.2亿（+54.4%）为增长最快板块，酵母蛋白新品类放量；"
        f"制糖13.4亿（+5.8%）毛利率-4.9%拖累综合毛利。"
    )
    if dom and ovs:
        p1 += (
            f"海外收入{ovs['amount']:.0f}M（占比{ovs['pct']:.1f}%，+19.9%）增速远超国内"
            f"（{dom['amount']:.0f}M，+4.1%），海外毛利率32.1%显著高于国内19.7%，全球化布局成效显著。"
        )

    # P2: 每股资金流向
    net_fcf = round(per_cf - per_capex - dps, 2) if per_cf else None
    p2 = (
        f"每股收益{_fmt(eps, 2)}元，每股经营现金流{_fmt(per_cf, 2)}元，资本支出每股{_fmt(per_capex, 2)}元（海外产能扩张期）。"
    ...
    p3 = (
        ...
        f"酵母行业核心壁垒：①规模与成本优势——全球第二大产能（49万吨），糖蜜采购议价力强，"
        f"规模效应摊薄固定成本；②渠道网络——覆盖170+国，海外6个生产基地本地化供应，"
        f"关税壁垒下竞争优势强化；③技术壁垒——菌种选育+发酵工艺Know-how，"
        f"YE（酵母抽提物）替代味精趋势明确，国内YE市占率60%+。"
        f"风险：糖蜜价格波动（占成本40%+）、海外汇率风险、制糖业务持续亏损拖累利润。"
    )
    ...
    p4 = (
        ...
        + f"，CF=20x对应{cf_20x}元。"
        f"历史PE区间23-41x，当前PE处于历史低位，具备安全边际。"
    )

    # P5: 催化剂与风险
    p5 = (
        "催化剂：①海外产能释放——埃及/俄罗斯工厂扩产，海外毛利率32%远高于国内，"
        "海外收入占比从41%向50%提升将结构性改善盈利；②酵母蛋白新品类——"
        "替代植物蛋白趋势，食品原料板块54%增速有望持续；③国内YE渗透率提升——"
        "酵母抽提物替代味精空间大，YE毛利率高于传统酵母。"
        "风险：糖蜜价格若重回高位将压缩毛利2-3pp；国内烘焙需求增速放缓；"
        "海外地缘政治风险（俄罗斯工厂）。关注每季度海外收入增速及酵母主业毛利率作为核心信号。"
    )
