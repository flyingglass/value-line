...
def build(stock, metrics, revenue_structure, years, cagr, spot):
    ...
    business = f"百润股份是中国预调鸡尾酒绝对龙头，旗下RIO锐澳品牌市占率超80%25。{latest_yr}年营收{rev:.0f}亿" + (f"（{_dir(rev, rev_prev)}{abs(rev_chg):.1f}%25）" if rev_chg else "") + f"，净利率{npm:.1f}%25，ROE {roe:.1f}%25。预调酒行业处于成长期，人均消费量远低于日美，渗透率提升空间大。"

    np_prev = py.get("HOLDER_PROFIT")
    np_chg = (np_val / np_prev - 1) * 100 if np_val and np_prev and np_prev > 0 else None

    p1 = f"{latest_yr}年营收约{rev:.0f}亿元" + (f"（{_dir(rev, rev_prev)}{abs(rev_chg):.1f}%25）" if rev_chg else "") + f"，扣非净利润约{np_val:.1f}亿元" + (f"（{_dir(np_val, np_prev)}{abs(np_chg):.1f}%25）" if np_chg else "") + f"。毛利率{gm:.1f}%25，净利率{npm:.1f}%25。预调酒产品毛利率高（60%25+），品牌壁垒深厚。"

    net_ps = round(per_cf - per_capex - dps, 2) if per_cf else None
    p2 = f"每股收益\u00a5{eps:.2f}。每股现金流\u00a5{per_cf:.2f}，资本支出\u00a5{per_capex:.2f}，现金分红\u00a5{dps:.2f}" + (f"（支付率{payout:.0f}%25）" if payout else "") + (f"，净留存\u00a5{net_ps:.2f}/股" if net_ps else "") + "。"

    p3 = f"毛利率{gm:.1f}%25、净利率{npm:.1f}%25、ROE {roe:.1f}%25。" + (f"ROIC {roce:.1f}%25。" if roce else "") + "竞争壁垒：①RIO品牌独占消费者心智，市占率80%25+近乎垄断；②渠道网络覆盖全国200万+终端，即饮+居家双场景覆盖；③预调酒品类渗透率提升是长期成长逻辑。风险：消费疲软、新进入者竞争、原材料成本波动。"

    pe = spot.get("pe", 0) or 0; pb = spot.get("pb", 0) or 0
    div_yield = spot.get("div_yield", 0) or 0
    p4 = f"当前PE约{pe:.1f}倍，PB约{pb:.1f}倍，股息率约{div_yield:.2f}%25。关注预调酒品类渗透率提升及新品放量节奏。"
