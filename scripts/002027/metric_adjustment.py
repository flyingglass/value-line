...
def adjust_metrics(reader, rd, np_val, tax_rate, stock_cfg):
    ...
    if year == "2025":
        ...
        footnotes.append(f"EPS adj: CAS ded {deducted/1e8:.1f} +Impair {abs(impair)/1e8:.1f} +EqLoss {SHUHE_EQUITY_LOSS/1e8:.1f} → VL {adj_np/1e8:.1f}亿 (数禾Q4一次性)")
    else:
        invest_income = reader.financial_item("income", "投资收益", rd) or 0
        adj_np = deducted - invest_income
        if abs(invest_income) > 0.5e8:
            footnotes.append(f"EPS adj: CAS ded {deducted/1e8:.1f} -InvInc {invest_income/1e8:+.1f} → VL {adj_np/1e8:.1f}亿")
