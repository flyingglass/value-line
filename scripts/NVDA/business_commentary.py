...
def build(stock, metrics, revenue_structure, years, cagr, spot):
    ...
    business = f"英伟达(NVIDIA)是全球AI算力芯片领导者，GPU市占率超80%25。{latest_yr}年营收${rev:.0f}亿" + (f"（{_dir(rev, rev_prev)}{abs(rev_chg):.1f}%25）" if rev_chg else "") + f"，净利率{npm:.1f}%25，ROE {roe:.1f}%25。数据中心业务为最大收入来源，AI大模型训练推理需求驱动高速增长。"

    np_prev = py.get("HOLDER_PROFIT")
    np_chg = (np_val / np_prev - 1) * 100 if np_val and np_prev and np_prev > 0 else None

    p1 = f"{latest_yr}财年营收约${rev:.0f}亿" + (f"（{_dir(rev, rev_prev)}{abs(rev_chg):.1f}%25）" if rev_chg else "") + f"，净利润约${np_val:.1f}亿" + (f"（{_dir(np_val, np_prev)}{abs(np_chg):.1f}%25）" if np_chg else "") + f"。毛利率{gm:.1f}%25，净利率{npm:.1f}%25。数据中心GPU需求持续爆发。"
    ...
    p3 = f"毛利率{gm:.1f}%25、净利率{npm:.1f}%25、ROE {roe:.1f}%25。" + (f"ROIC {roce:.1f}%25。" if roce else "") + "竞争壁垒：①CUDA生态锁定全球AI开发者；②每年一代GPU架构迭代速度碾压竞品；③与台积电深度绑定先进制程产能。风险：AI资本开支周期性、出口管制、竞争加剧。"

    pe = spot.get("pe", 0) or 0; pb = spot.get("pb", 0) or 0
    div_yield = spot.get("div_yield", 0) or 0
    p4 = f"当前PE约{pe:.1f}倍，PB约{pb:.1f}倍，股息率约{div_yield:.2f}%25。关注AI Capex趋势及Blackwell/下一代GPU出货节奏。"
