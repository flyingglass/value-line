...
def build(stock, metrics, revenue_structure, years, cagr, spot):
    ...
    biz=f"泡泡玛特是中国领先的潮流文化娱乐公司，以IP为核心，覆盖艺术家发掘、IP运营、全球零售及粉丝社区。"+(f"品类：{prod_str}。" if prod_str else "产品涵盖毛绒、手办、盲盒、MEGA珍藏、衍生品等全品类。")+f"最新财年营收{_fmt(rev,0)}亿（{_dir(rev,py.get('OPERATE_INCOME'))}{abs(r_chg):.1f}%），净利率{_p(npm)}，ROE {_p(roe)}。全球化布局加速，海外门店覆盖30+国家。"
