...
def build(stock, metrics, revenue_structure, years, cagr, spot):
    ...
    if isinstance(revenue_structure, dict):
        for dim_key, dim_label in [("by_product", "产品涵盖"), ("by_channel", "渠道"), ("by_region", "覆盖")]:
            items = revenue_structure.get(dim_key, [])
            if items and len(items) >= 2:
                names = "、".join([f"{r['name']}{r['pct']}%" for r in items[:4]])
                rev_struct_hint = f"。{dim_label}{names}" if names else ""
                break

    desc = stock.get("business_desc", "")
    biz = f"{desc} {latest_yr}年营收{rev:.0f}亿" + (f"（{_dir(rev, rev_prev)}{abs(rev_chg):.1f}%）" if rev_chg else "") + f"，净利率{npm:.1f}%，ROE {roe:.1f}%。{rev_struct_hint}" if desc else f"{latest_yr}年营收{rev:.0f}亿，净利率{npm:.1f}%，ROE {roe:.1f}%。{rev_struct_hint}"
