...
- **🔴 落库位置（2026-09-25 用户判定）**：`raw/` ＝ **只进不改的原始资料原件**（业绩会逐字稿、调研/研报/文章/PDF 原文）；**二次加工稿**（披露摘录、汇总表、含推算的证据汇编）不论多"原始"一律进 `research/<标的>/`。标的目录用**中文简称**（如 `中航信`），**不用证券代码**（曾误建 `raw/research/00696/`，被要求移出且改名）
脚本：`.venv/Scripts/python.exe scripts/wiki_lint.py`（健康检查）/ `scripts/generate_wiki_index.py`（重建）

## 2444 归因纪律（2026-09-25 用户驳回教训）
- **六类变化是有层级的**：类 4（预期/估值）是**认知层**，只放"市场怎么看"的变化，**不许填利润表数字**；类 2 才是业绩/基本面层。混层 = 硬错误（中航信把"网费 -8.9%"归入类 4 被批"搞笑"）
- **细项不得指认整体方向**：不能用占营收 6% 的板块内分项（如网费）推断公司"基本面下行"。双左侧的判定要件只有一条口径：**基本面整体往下 + 股价大走势往下**，用主业量价指标（处理量/单价/结算笔数/毛利率/净利）
- **慢变量不许当既成事实**：尚未发生的长期风险（如 NDC 侵蚀）放"证伪指标清单"并配阈值，不得写成"当期反向信号"
- **应看的主业量价**：平台费单价 = 平台服务收入 ÷ 系统处理量（2025 = 6.02 元/人次 vs 2024 ≈ 6.18，🟡推算 -2.6%）——量涨价跌才是"分销费见顶"的第一手读数
- **wiki 缺口**：`research/` 下无中航信（00696）目录 → 按 lihai-2444-analysis 硬约束（只用 wiki 沉淀页 + DB）出不了完整两期对照报告
