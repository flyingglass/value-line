...
- ⚠️ **2026-09-11 强化**：上述规则适用于**仓库内一切文件** —— 包括 `.codebuddy/skills/*/SKILL.md`、文档、配置，**不只代码**。
  - 纯问答场景（"X 怎么处理？""Y 如何打开？"）**只回答，不动任何文件**；有改进想法 → 作为**提案**写在回复里，等用户点头
  - 踩坑记录：曾两次答完问题顺手改 skill md，被用户当场指出"没说需要修改 skill md 呀？"
- 给出方案→等用户确认→执行→确认结果→下一步
- 中报数据标记"仅AKShare"来源
- Memory放项目repo可随git提交
- 🔴 **wiki 页面禁止「过程注脚 / 对账叙述」（2026-09-19 固化，用户三轮同类诉求）**：
  页面正文只放**当前有效的结论与名单**，不写「某日更新①②③」「与 X 表删除线逐一对齐（N 家 = …＋…）」
  「原某节清零并入本节」这类**变更日志式**文字——它们属于 `log.md`，不属于页面。
  家数只写在**标题行/计数行**（如「二、已扫描 · 有结论（169 家）」「2.1 候选 · 跟踪（35 家）」），
  不在正文重复解释；口径说明只在**表内逐行**体现（划掉口径列 / 依据列）。
  历史迭代：log 3638 行（删 2.1 末尾「点名记录」注脚）→ 2026-09-19 第一轮（删头部 7 个过程注脚块 13 行）
  → 2026-09-19 第二轮（删 ②节/2.2 节头部 4 个引用块 8 行）。
  ⚠️ 反复出现的病根：**每轮改动都把「这次改了什么」追加进页面**，导致页面越滚越长且新旧家数自相矛盾
  （实测第 7 行同时写 pass 134 家 与 2.2 现 74 家）。改成：**页面只被覆盖，只进不改的是 raw，变更记 log**。
...
**归档类名单独立成页（2026-09-19 固化）**：主表只留「**当前有效集合**」，只进不改的归档单独成页。
- 先例：`广州待选池-依据存档.md`（2026-09-17 由跟踪表第五节拆出）、
  `广州名单-已划掉（134 家）.md`（2026-09-19 由跟踪表 2.2 节整表拆出）
- 主表（`广州名单-待选池跟踪表.md`）现只含：一、打分口径 → 二、候选 · 跟踪（35 家）→
  四、边界清单（26 家）→ 五、依据与存档（放指向归档页的**导航链接**，不为省事留 pass 名单）
- 拆表时注意：段内「（第五节）」这类**页内位置引用**在搬家后失去所指，须改写为「（见依据存档）」
- 节号可能出现跳号（一/二/四/五，原「三」已删），**不重新编号**——表内单元格引用了「第五节」等位置
...
- **Bash 命令替换吞 Markdown（2026-09-19 踩坑）**：用 `python -c "…多行脚本…"` 且外层是 bash **双引号**时，脚本字符串里的
  反引号被当命令替换执行、`>` 开头行被当重定向。
  🔴 最危险的一层：反引号内若是**相对路径且该文件真实存在**（如 `学股/广州/xxx.md`），
  shell 因 ENOEXEC 会回退用 sh **把那个 md 文件当脚本执行** → 文件里 `> **来源**：…` 那行触发重定向，
  凭空创建垃圾文件（实测误建了一个 0 字节的 `**来源**：` 文件在 `research-wiki/research/`）。
  - 规避：① 多行文本先用 **Write 工具**落成文件再让 python 读，或 ② bash 里用**单引号**包裹 python 代码（不可插值但安全），
    或 ③ 脚本内避免出现反引号
  - 检测：`git status --short` 里冒出 `??` 的异常文件名 = 中招；立刻用 python `os.remove` 精确删除
  - 教训：**不要用 bash 双引号 + `python -c` 写含 Markdown 语法的长文本**（本项目 log/记忆条目全是 Markdown，极易触发）
