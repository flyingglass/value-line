...
## [2026-09-10] lint | research-wiki 全站健康检查 — 结构项全部清零（frontmatter / 参见 / 孤立 / 注册）

### 操作
- 运行 `scripts/wiki_lint.py` 全量检查 291 个 .md（raw 78 / vl 37 / research 175）
- 升级 `scripts/wiki_fix.py`：修复日期更新为 2026-09-10；新增 vl/modules、疯狂的里海专题（时间线/案例/方法论）frontmatter 推断；标的子页按中英文文件名映射 category；交叉引用批量追加改为 `--crossref` 显式开启（默认不做，避免一次性改动上千行）
- 修正 `scripts/wiki_lint.py` 两处误报：跳过 `[[#页内锚点]]`；研究专题目录（疯狂的里海）不再按标的四件套校验

### 修复（52 页）
- **frontmatter 缺失 20 → 0**：疯狂的里海时间线 ×11、疯狂的里海.md、里海-2444投资体系.md、建滔集团 ×3、建滔积层板 ×2、巴芒论迪士尼、vl/modules/generate_wiki_index.py.md
- **`## 参见` 缺失 32 → 0**：泡泡玛特业绩 ×14、疯狂的里海 ×13、拼多多/阿里巴巴/贵州茅台/芒格格栅系 ×5（参见一律使用全局唯一页面名，避免 stem 兜底歧义）
- **孤立页面 13 → 0 / index 未注册 17 → 0**：业绩会纪要索引补 12 页入链；research/index.md 注册泡泡玛特业绩 12 页 + 经营时间序列 + 跟踪 02/03 + 里海年度时间线-2018 年 + 盲眼钟表匠
- **断链修复 5 处**：`投研框架-复杂经济学指导手册` → `投资框架-…`（1）；`[[03-环节演变时序]]` → `[[03-环节演变时序（2020-2026H1）]]`（3）；移除指向已清理 raw 的失效链接（1）

### 剩余（需内容决策，未自动处理）
- **ERROR 19**：TME / 拼多多 / 阿里巴巴 缺标的四件套（各 4 页，当前仅业绩子目录）；建滔集团缺 operating-metrics；建滔积层板缺 industry-chain/operating-metrics；贵州茅台缺 数据目录/投资论点；分众传媒缺 产业链/投资论点
- **断链 71**：raw/ 内约 24 处（缺概念页：均值回归/复杂适应系统/思维格栅模型/行为金融学/贝叶斯定理/Lollapalooza 效应等，只读不可改）；`research/log.md`、`vl/log.md` 历史改名文本（按只追加规则不改）；`时代天使/研报索引` 与 `里海案例时间线-秦安/太极/鲁西/再升`（页不存在的历史条目，log 已两次标注待清理）
- **交叉引用缺口 589**（527 → 589：新增参见区块本身带来新入链，符合 diminishing returns 结论）
- **占位符 INFO 9**（正常待补标注）、**raw 命名 INFO 5**、**vl/synthesis/ 空目录**

### 追加（同日·口径收窄）
- `wiki_lint.py`：`raw/` 与各级 `log.md` 不再做断链校验（前者只进不改、后者只追加，历史路径必然含旧名）；交叉引用缺口收窄为「同分组（同标的 / 同专题 / 同 vl·articles 子类）内强制双向 + 容器页（index/overview/log）豁免」
- 效果：断链 71 → **14**、交叉引用缺口 589 → **224**、WARN 662 → **239**
- 剩余 14 处断链全部为内容缺口：缺概念页 4（均值回归 / 思维格栅模型 / 行为金融学 / 贝叶斯定理）、缺页 2 类（`时代天使/研报索引`、`里海案例时间线-秦安/太极/鲁西/再升`——后者 log 已两度标记待清理）

### 收尾（同日·失效引用清理 + 同组缺口补全）
- 清理失效引用：`时代天使/研报索引`（index 条目 + 概览/投资论点/竞争时间线 共 4 处）、`里海案例时间线-秦安/太极/鲁西/再升`（index 4 条：文件不存在，属目录迁移遗留，log 已两度标记待清理）、index 日志行的 `[[疯狂的里海/…]]` 省略写法
- 跑 `python scripts/wiki_fix.py --crossref` 补同组反引 **165 项**
- 结果：断链 **14 → 4**、交叉引用缺口 **224 → 166**、WARN **239 → 171**
- 保留：4 个缺失概念页（均值回归 / 思维格栅模型 / 行为金融学 / 贝叶斯定理）——经确认**不建页**，作为已知缺口长期保留（raw 侧同目标断链已随 raw 豁免一并静默）

### 状态
- 结构类问题全部清零：frontmatter / 参见区块 / 孤立页面 / index 注册
- 内容类问题（缺页、缺失概念页、交叉引用缺口）保留，按历史惯例不做自动补页

触及页面：research/index.md、research/泡泡玛特/业绩/*（14）、research/疯狂的里海/*（13）、research/建滔集团|建滔积层板（5）、research/articles/*（4）、scripts/wiki_lint.py、scripts/wiki_fix.py
