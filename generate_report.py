...
(function(){{
  ...
  html+='<table style="width:100%;border-collapse:collapse;font-size:9px;line-height:1.35;margin:1px 0">';
  ...
  setTimeout(function(){{
    ...
    var tooltipFmt=function(p){{var di=p[0].dataIndex,h=p[0].axisValue,r='<b>'+h+'</b>',mk=p[0].marker;var dk=kl.find(function(k){{return k.date===h;}});if(dk)r+='<br/>'+mk+' '+stockName+'<br/> open:'+dk.open.toFixed(2)+'<br/> close:'+dk.close.toFixed(2)+'<br/> low:'+dk.low.toFixed(2)+'<br/> high:'+dk.high.toFixed(2);for(var i=1;i<p.length;i++){{var v=p[i].value,n=p[i].seriesName;if(v==null)continue;if(n==='Rel Strength')r+='<br/>'+p[i].marker+' '+n+': '+(v!=null?Number(v).toFixed(1):'-');else if(n.indexOf('15x CF')>-1)r+='<br/>'+p[i].marker+' '+n+': '+Math.exp(Number(v)).toFixed(2);}}var pv=volData[di];if(pv!=null){{var up=dk&&dk.close>dk.open;r+='<br/><span style=\"display:inline-block;width:8px;height:8px;border-radius:50%;background:'+(up?'#ef232a':'#14b143')+';margin-right:4px;vertical-align:middle\"></span>PST: '+pv.toFixed(2)+'%';}}return r;}};
